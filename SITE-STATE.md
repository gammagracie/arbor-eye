...
- **Main image**: on desktop it fits the height between header and footer and is centred, never wider than the content; on phones/tablets it also fits the screen height so the footer is always visible and the image-plus-thumbnails block is vertically centred. The image sits in a fixed 1.494:1 frame (object-fit cover, so all nine works share one size and nothing shifts on change) and fades out/in over 0.4s each way; all carousel images are preloaded; the Arbor & Eye insignia (`uploads/arbor-mark.png`, grey line drawing) sits behind it and shows through during the fade.
...
- **Filmstrip**: every visible work in one row anchored above the footer, thumbnails 64px tall (52px on phones) with 10px gaps (files in `assets/thumbs/`, 260px tall so they stay sharp when magnified), edges faded. On mouse devices the row is a track moved by an eased transform: the cursor's position across the screen sets where the row sits (left = start, right = end) and it glides there (a slow drift: the row eases up to a top speed of 380px per second, about three and a half thumbnails a second, cruises, then eases to rest, so crossing the whole strip takes about 12s; it coasts to a stop when the cursor leaves the strip; identical on 60Hz and 120Hz screens; `CRUISE`, `GLIDE` and `ACCEL` in the page script); the wheel/trackpad also moves it, directly and without the speed limit. Thumbnails magnify Dock-style under the cursor (up to about 190%, bell-curve falloff, growing up from the baseline, neighbours sliding aside, the one under the cursor staying under it) and relax when the cursor leaves the strip. Touch mode: the strip scrolls natively under the finger with no magnification or mouse glide, with 16px of clear space above the thumbnails so nothing is clipped; it only moves while it is being swiped (normal flick momentum, then rest), and a tap anywhere else on the page stops it dead. The page picks mouse or touch mode from the input actually used (any touch switches to touch mode, real mouse movement switches back, the row keeping its place), so phones that wrongly report a mouse still behave as touch devices. Click a thumbnail to show that work; the active one is outlined, the rest dimmed. Arrow keys and finger swipes on the stage step through works.
