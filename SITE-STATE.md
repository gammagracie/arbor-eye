...
- **Stage**: the selected work shown whole, as large as the window allows: it fills the height between header and filmstrip (16px below the header) and, from 768px up, may run wider than the page's usual side padding, out to 3% of the screen width each side with the chevrons still hugging it, centred on the off-white page with the usual soft shadow; works crossfade (0.6s) with the insignia behind. First work (Brilliance) on load, or the work named in the URL hash.
