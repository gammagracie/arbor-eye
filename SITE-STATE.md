...
- **Main image**: on desktop and tablet the header, image and thumbnails fill the screen exactly: the thumbnails sit at the bottom of the viewport, with room reserved beneath them for the hover title (10px gap, one line, then 20px clear to the bottom edge), and the image fills everything between them and the header (28px gap, 40px on tablet); the footer follows below the fold and is reached by scrolling. The image may run wider than the page's side padding, up to 94% of the screen width, so it can still fill the height on very large or squarer screens; where even that is not enough (tall, narrow windows) the image-plus-thumbnails block is centred. On phones the footer stays on screen and the block is vertically centred, as the image is limited by width there. The image sits in a fixed 1.494:1 frame (object-fit cover, so all nine works share one size and nothing shifts on change) and fades out/in over 0.4s each way; all carousel images are preloaded; the Arbor & Eye insignia (`uploads/arbor-mark.png`, grey line drawing) sits behind it and shows through during the fade.
- **Header and footer alignment (home only)**: on desktop and tablet the logo and the copyright line start on the painting's left edge, and the nav/icons and the social icons end on its right edge, at every screen size (so they move outward on large screens and inward on laptop screens). Limits: never closer than 24px to the screen edge, and never narrower than the nav needs (960px desktop, 700px tablet), so on very short, wide windows the header stays wider than the painting. Phones use the normal side padding. Other pages keep the standard side padding.
