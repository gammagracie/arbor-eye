...
- **Main image**: on desktop it fits the height between header and footer and is centred, never wider than the content; on phones/tablets it also fits the screen height so the footer is always visible and the image-plus-thumbnails block is vertically centred. Image fades out/in (0.2s) on change; the Arbor & Eye insignia (`uploads/arbor-mark.png`, grey line drawing) sits behind it and shows through during the fade.
